...
The repo is mid-way through a modernization pass — see `dispatch/docs/modernization-runbook.md` for the staged plan and current status, and `dispatch/docs/daily-soccer-digest.md` for an in-progress second command (`/digest`, not yet implemented — `dispatch/tasks/soccer.go` is currently a placeholder stub). Check those docs before assuming something is finished; several correctness fixes they describe (Phase 3: nil-deref risk in `telegram.Send`, in-memory-only dedupe DB, hardcoded `bot.Debug`) are still open in the code as of this writing.
...
- `dispatch/store/` — persistence via GORM over SQLite. `entities.go` defines `RedditPost` (a `PostID` field plus GORM's default model fields) used to dedupe posts already seen — `PostID` holds the RSS entry's `<id>` (e.g. `t3_1vuw4s6`), not a URL, despite the struct's name. `store.go`'s `Init()` opens an **in-memory** SQLite DB (`file::memory:?cache=shared`) and auto-migrates the schema — meaning dedupe state does not persist across process restarts. `Exists(postID)` / `Create(postID)` each call `Init()` internally (opens a fresh connection per call rather than reusing a shared handle).
...
- `dispatch/telegram/telegram.go`'s `Send` doesn't return after a failed `NewBotAPI` call before dereferencing `bot.Self.UserName` — a real nil-deref risk on a bad token, tracked as Phase 3a in the modernization runbook, not yet fixed.
- Because the SQLite DB is in-memory, dedupe (`RedditPost` records) resets every time the bot restarts.
